...
export default function DataInputTab() {
  ...
  const handleCsvImport = async () => {
    if (!csvCampaignId || !csvClientId) { setErrorMsg('Pilih client dan campaign untuk import'); return }
    ...
    const rows = csvData.map(row => ({
      ...
      report_date: row.report_date || row.date || '',
    ...
    if (error) setErrorMsg('Gagal import: ' + error.message)
    else {
      setSuccessMsg(`${rows.length} baris berhasil diimport!`)
      setCsvData([]); setCsvFileName(''); setCsvCampaignId(''); setCsvClientId(''); setCsvPlatform('')
      fetchRecentData()
    }
    setLoadingCsv(false)
  }
    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
              ...
              {PLATFORMS.map(p => {
                ...
                return (
                  <button key={p} onClick={() => set('platform', p)}
                    style={{
                      flex: 1, padding: '7px 4px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: isActive ? 500 : 400,
                      border: isActive ? `1.5px solid ${colors[p].active}` : '0.5px solid rgba(0,0,0,0.12)',
                      background: isActive ? colors[p].bg : '#fff',
                      color: isActive ? colors[p].active : '#555',
                    }}>{p}</button>
                )
              })}
            </div>
      ...
      <Card title="Import CSV dari Ads Manager">
        <div style={{ fontSize: '11px', color: '#888', marginBottom: '14px' }}>
          Format kolom CSV: <code style={{ background: '#f5f5f3', padding: '1px 6px', borderRadius: '4px' }}>report_date, impressions, reach, clicks, spend, conversions_7d_click, conversion_value, purchases</code>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: '14px', marginBottom: '14px' }}>
          <div>
            <Label required>1. Client</Label>
            <SelectField value={csvClientId} onChange={v => { setCsvClientId(v); setCsvPlatform(''); setCsvCampaignId('') }}
                ...
                return (
                  <button key={p} onClick={() => { setCsvPlatform(p); setCsvCampaignId('') }}
                    style={{
                      flex: 1, padding: '7px 4px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: isActive ? 500 : 400,
                      border: isActive ? `1.5px solid ${colors[p].active}` : '0.5px solid rgba(0,0,0,0.12)',
                      background: isActive ? colors[p].bg : '#fff', color: isActive ? colors[p].active : '#555',
                    }}>{p}</button>
                )
              })}
            </div>
          ...
          <div>
            ...
            <SelectField value={csvCampaignId} onChange={setCsvCampaignId}
              ...
              placeholder={!csvClientId || !csvPlatform ? '— Pilih client & platform dulu —' : '— Pilih Campaign —'} />
          </div>
        </div>
        <div style={{ marginBottom: '14px' }}>
          <Label>Upload file CSV</Label>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginTop: '4px' }}>
            <label style={{ padding: '8px 16px', background: '#f5f5f3', border: '0.5px solid rgba(0,0,0,0.15)', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', color: '#555' }}>
              Pilih File
              <input type="file" accept=".csv" style={{ display: 'none' }} onChange={handleCsvUpload} />
            </label>
            {csvFileName && <span style={{ fontSize: '11px', color: '#888' }}>{csvFileName} — {csvData.length} baris</span>}
          </div>
        </div>

        {csvData.length > 0 && (
          <div style={{ marginBottom: '14px', overflowX: 'auto', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: '8px' }}>
            <div style={{ fontSize: '11px', color: '#888', padding: '8px 10px', borderBottom: '0.5px solid rgba(0,0,0,0.08)' }}>Preview (5 baris pertama dari {csvData.length} total)</div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              <thead>
                <tr>{Object.keys(csvData[0]).map(h => <th key={h} style={{ padding: '6px 10px', background: '#f5f5f3', textAlign: 'left', borderBottom: '0.5px solid rgba(0,0,0,0.08)', fontWeight: 500, color: '#888', whiteSpace: 'nowrap' }}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {csvData.slice(0, 5).map((row, i) => (
                  <tr key={i}>{Object.values(row).map((v, j) => <td key={j} style={{ padding: '5px 10px', borderBottom: '0.5px solid rgba(0,0,0,0.05)', whiteSpace: 'nowrap' }}>{v}</td>)}</tr>
                ))}
              </tbody>
            </table>
          </div>
